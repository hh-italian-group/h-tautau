...
double LeptonWeights::GetIdIsoWeight(EventInfoBase& eventInfo) const
{
    ...
    if(channel == Channel::ETau) {
        return electronSF.GetIdIsoSF(eventInfo.GetLeg(1).GetMomentum()) * tauIdWeight->GetIdIsoSF(LorentzVectorM_Float(eventInfo.GetLeg(2).GetMomentum()),
            static_cast<GenLeptonMatch>(event.lep_gen_match.at(eventInfo.GetLegIndex(2))), event.lep_decayMode.at(eventInfo.GetLegIndex(2)), DiscriminatorWP::Tight,
        ...
        return muonSF.GetIdIsoSF(eventInfo.GetLeg(1).GetMomentum()) * tauIdWeight->GetIdIsoSF(LorentzVectorM_Float(eventInfo.GetLeg(2).GetMomentum()),
            static_cast<GenLeptonMatch>(event.lep_gen_match.at(eventInfo.GetLegIndex(2))), event.lep_decayMode.at(eventInfo.GetLegIndex(2)),  DiscriminatorWP::VLoose,
        ...
        return tauIdWeight->GetIdIsoSF(LorentzVectorM_Float(eventInfo.GetLeg(1).GetMomentum()),
            static_cast<GenLeptonMatch>(event.lep_gen_match.at(eventInfo.GetLegIndex(1))), event.lep_decayMode.at(eventInfo.GetLegIndex(1)), DiscriminatorWP::VLoose,
    ...
    if(channel == Channel::ETau) {
        if(electronSF.HasCrossTriggers() && std::abs(eventInfo.GetLeg(2).GetMomentum().eta()) < 2.1){
            ...
            const double tau_eff = tauTriggerWeight->GetEfficiency(channel, LorentzVectorM(eventInfo.GetLeg(2).GetMomentum()),
                                                                   static_cast<GenLeptonMatch>(event.lep_gen_match.at(eventInfo.GetLegIndex(2))),
                                                                   event.lep_decayMode.at(eventInfo.GetLegIndex(2)), tau_iso_wp, isData);
        ...
        double tauSF_1 = tauTriggerWeight->GetEfficiency(channel, LorentzVectorM(eventInfo.GetLeg(1).GetMomentum()), static_cast<GenLeptonMatch>(event.lep_gen_match.at(eventInfo.GetLegIndex(1))),
                                               event.lep_decayMode.at(eventInfo.GetLegIndex(1)), tau_iso_wp, isData);
        double tauSF_2 = tauTriggerWeight->GetEfficiency(channel, LorentzVectorM(eventInfo.GetLeg(2).GetMomentum()), static_cast<GenLeptonMatch>(event.lep_gen_match.at(eventInfo.GetLegIndex(2))),
                                                   event.lep_decayMode.at(eventInfo.GetLegIndex(2)), tau_iso_wp, isData);
