...
class TauIdWeight2017 : public IWeightProvider {
public:
    using Event = ntuple::Event;
    using ptree = boost::property_tree::ptree;
    ...
    TauIdWeight2017(const std::string& tauId_input, DiscriminatorWP _iso_wp) : iso_wp(_iso_wp)
    {
        ptree property_tree;
        boost::property_tree::json_parser::read_json(tauId_input, property_tree);
        for (auto& type_entry : property_tree) {
    ...
    virtual double Get(const Event& event) const override
    {
        ...
        if(channel == Channel::ETau)
            return EvaluateSF(event.p4_2.pt(), static_cast<GenMatch>(event.gen_match_2), event.decayMode_2);
    ...
    virtual double Get(const ntuple::ExpressEvent& /*event*/) const override
    {
        throw exception("ExpressEvent is not supported in TauIdWeight2017::Get.");
    }
