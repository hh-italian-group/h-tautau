...
#include "Analysis/include/TupleObjects.h"
...
public:
    ...
    using analysis::UncertaintySource = UncertaintySource;
    using analysis::UncertaintyScale = UncertaintyScale;
    ...
    JECUncertaintiesWrapper(const std::string& uncertainties_source) {
       ...
       for (int isrc = 0; isrc < JetUncertainties().size(); isrc++) {
           std::string full_name = analysis::ToString(JetUncertainties_withTotal().at(isrc));
        ...
        analysis::UncertaintyScale scale, MET* met = nullptr)
    {
        ...
        if(!uncertainty_map.count(uncertainty_source))
            throw exception("Jet Uncertainty source % not found.") % unc_source;
        if(scale == analysis::UncertaintyScale::Central)
            throw exception("Uncertainty scale Central.");
        ...
        for (const auto& jet : jet_candidates){
            ...
            const double sf = (1.0 + (scale * unc_var));
        ...
        met->SetPxPyPzE(shifted_met_px,shifted_met_py,0,0);
