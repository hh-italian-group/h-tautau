...
#include "h-tautau/JetTools/include/JetCorrectorParameters.h"
...
JetCorrectorParameters::Definitions::Definitions() : mIsResponse(false) {}

JetCorrectorParameters::Definitions::Definitions(const std::vector<std::string>& fBinVar,
    const std::vector<std::string>& fParVar, const std::string& fFormula, bool fIsResponse)
{
...
unsigned JetCorrectorParameters::Definitions::nBinVar() const { return unsigned(mBinVar.size()); }
unsigned JetCorrectorParameters::Definitions::nParVar() const { return unsigned(mParVar.size()); }
std::vector<std::string> JetCorrectorParameters::Definitions::parVar() const { return mParVar; }
std::vector<std::string> JetCorrectorParameters::Definitions::binVar() const { return mBinVar; }
std::string JetCorrectorParameters::Definitions::parVar(unsigned fIndex) const { return mParVar[fIndex]; }
std::string JetCorrectorParameters::Definitions::binVar(unsigned fIndex) const { return mBinVar[fIndex]; }
std::string JetCorrectorParameters::Definitions::formula() const { return mFormula; }
std::string JetCorrectorParameters::Definitions::level() const { return mLevel; }
bool JetCorrectorParameters::Definitions::isResponse() const { return mIsResponse; }

JetCorrectorParameters::Record::Record() : mNvar(0),mMin(0),mMax(0) {}
...
JetCorrectorParameters::Record::Record(unsigned fNvar, const std::vector<float>& fXMin,
    const std::vector<float>& fXMax, const std::vector<float>& fParameters) :
    mNvar(fNvar),mMin(fXMin),mMax(fXMax),mParameters(fParameters) {}

float JetCorrectorParameters::Record::xMin(unsigned fVar) const { return mMin[fVar]; }
float JetCorrectorParameters::Record::xMax(unsigned fVar) const { return mMax[fVar]; }
float JetCorrectorParameters::Record::xMiddle(unsigned fVar) const { return float(0.5*(xMin(fVar)+xMax(fVar))); }
float JetCorrectorParameters::Record::parameter(unsigned fIndex) const { return mParameters[fIndex]; }
std::vector<float> JetCorrectorParameters::Record::parameters() const { return mParameters; }
unsigned JetCorrectorParameters::Record::nParameters() const { return unsigned(mParameters.size()); }
int JetCorrectorParameters::Record::operator< (const Record& other) const { return xMin(0) < other.xMin(0); }

JetCorrectorParameters::JetCorrectorParameters() { valid_ = false;}
...
const JetCorrectorParameters::Record& JetCorrectorParameters::record(unsigned fBin) const {return mRecords[fBin]; }
const JetCorrectorParameters::Definitions& JetCorrectorParameters::definitions() const {return mDefinitions;   }
unsigned JetCorrectorParameters::size() const {return unsigned(mRecords.size());}
...
bool JetCorrectorParameters::isValid() const { return valid_; }
...
JetCorrectorParameters const & JetCorrectorParametersCollection::operator[]( key_type k ) const
{
    ...
    throw analysis::exception("InvalidInput, cannot find key '%1%' in the JEC payload, this usually means you have"
                              " to change the global tag") % static_cast<int>(k);
}
...
{
    keys.clear();
    for ( collection_type::const_iterator ibegin = corrections_.begin(), iend = corrections_.end(), i = ibegin;
          i != iend; ++i ) {
...
{
    return k == L5Flavor || ( k / 100 > 0 && k / 1000 == 0 );
}

bool JetCorrectorParametersCollection::isL7( key_type k )
{
    return k == L7Parton || ( k / 1000 > 0 );
}
